...
# Scenario configuration (excluding ASR disorder diagnosis)
SCENARIOS = {
    'disorder_diagnosis': {
...
# Define pairs of scenarios for the two figures
FIGURE_PAIRS = [
    ['disorder_diagnosis', 'disorder_type_classification'],
    ['disorder_symptom_classification', 'transcription_accuracy']
]
...
def create_paired_plots():
    """
    Create two figures, each with two subplots showing scenario pairs.
    """
    ...
    for fig_idx, scenario_pair in enumerate(FIGURE_PAIRS):
        # Create figure with 1x2 subplots
        fig, axes = plt.subplots(1, 2, figsize=(10, 5))
        
        for i, scenario_key in enumerate(scenario_pair):
            ...
            plot_scenario(
                ...
                show_labels=(i == 0)  # Show labels only for first subplot in each figure
            )
        
        plt.tight_layout(pad=2.0, h_pad=3.0, w_pad=3.0)
        
        # Save figure with dpi=100
        filename = f"figure_{fig_idx + 1}_{'_'.join(scenario_pair)}.png"
...
# Create and save the paired plots
figures = create_paired_plots()
